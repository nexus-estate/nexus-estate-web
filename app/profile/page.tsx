...
export default function ProfilePage() {
  ...
  function populateEditFields(u: User) {
    ...
    setEditDateOfBirth(u.profile?.dateOfBirth ? u.profile.dateOfBirth.split('T')[0] : '');
  ...
  }, [authLoading, isAuthenticated, router, loadUser]);
          ...
          <>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm mb-6">
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                ...
                <div className="flex-1 text-center sm:text-left">
                  <h1 className="text-2xl font-bold text-gray-900">{getUserFullName(user)}</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2">
                {editMode ? (
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                    <h2 className="text-lg font-semibold text-gray-900 mb-6">Chỉnh sửa thông tin</h2>
                    {error && (
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <dl className="divide-y divide-gray-100">
                      <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        ...
                        <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{getUserFullName(user)}</dd>
                      </div>
                      <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                      ...
                      <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        ...
                        <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{user.profile?.phone || '---'}</dd>
                      </div>
                      <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt className="text-sm font-medium text-gray-500">Vai trò</dt>
                        <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 capitalize">{user.role?.toLowerCase()}</dd>
                      </div>
                      <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt className="text-sm font-medium text-gray-500">Ngày sinh</dt>
                        <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{formatDate(user.profile?.dateOfBirth)}</dd>
                      </div>
                      <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt className="text-sm font-medium text-gray-500">Địa chỉ</dt>
                        <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                          {[user.profile?.address, user.profile?.city, user.profile?.country].filter(Boolean).join(', ') || '---'}
                        </dd>
                      </div>
                      <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt className="text-sm font-medium text-gray-500">Tiểu sử</dt>
                        <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{user.profile?.bio || '---'}</dd>
                      </div>
                      <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                      ...
                      <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        ...
                        <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{formatDate(user.updatedAt)}</dd>
                      </div>
                    </dl>
                  </div>
                )}
              ...
              <div className="space-y-6">
                {/* Stats Card */}
                <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                  <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Thống kê</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-xl bg-blue-50 p-4 text-center">
                      <p className="text-2xl font-bold text-blue-700">{user.profile?.followersCount || 0}</p>
                      <p className="text-xs text-blue-600 mt-0.5">Người theo dõi</p>
                    </div>
                    <div className="rounded-xl bg-purple-50 p-4 text-center">
                      <p className="text-2xl font-bold text-purple-700">{user.profile?.followingCount || 0}</p>
                      <p className="text-xs text-purple-600 mt-0.5">Đang theo dõi</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                  <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Bảo mật</h3>
                  {!showPasswordForm ? (
                    <button
                    ...
                    <form onSubmit={handleChangePassword} className="space-y-4">
                      ...
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Mật khẩu cũ</label>
                        <input
                          type="password"
                          value={oldPassword}
                ...
                {/* Quick Links */}
                <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                  <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Liên kết nhanh</h3>
                  <nav className="space-y-2">
                    <Link href="/dashboard" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                      <svg className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                      </svg>
                      Dashboard
                    </Link>
                    <Link href="/dashboard/listings/new" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                      <svg className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                      </svg>
                      Đăng tin mới
                    </Link>
                    <Link href="/properties" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                      <svg className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
