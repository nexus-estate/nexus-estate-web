    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <Link href="/properties" className="mt-2 inline-block text-blue-600 hover:underline">Quay lại</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <nav className="mb-6 text-sm text-gray-500">
          ...
          <Link href="/properties" className="hover:text-blue-600">Nhà đất</Link>
          <span className="mx-2">/</span>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex h-72 sm:h-96 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-700 text-white">
              <div className="text-center">
                <span className="text-8xl opacity-40">
                  {p.type === 'apartment' ? '🏢' : p.type === 'house' ? '🏠' : p.type === 'villa' ? '🏡' : '🗺️'}
                </span>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {p.purpose === 'buy' ? 'Bán' : 'Cho thuê'} &middot; {p.type === 'apartment' ? 'Căn hộ' : p.type === 'house' ? 'Nhà phố' : p.type === 'villa' ? 'Biệt thự' : 'Đất nền'}
              </p>
              ...
              <div className="mt-4 flex flex-wrap gap-4 border-y border-gray-100 py-4">
                ...
                {p.bedrooms && (
                  <div className="text-center">
                    <p className="text-lg font-bold text-gray-900">{p.bedrooms}</p>
                    <p className="text-xs text-gray-500">Phòng ngủ</p>
                  </div>
                )}
                {p.bathrooms && (
                  <div className="text-center">
                    <p className="text-lg font-bold text-gray-900">{p.bathrooms}</p>
              ...
              {p.broker && (
                <div className="mt-6 rounded-xl bg-gray-50 p-4">
                  <h2 className="font-semibold text-gray-900">Thông tin môi giới</h2>
                  <p className="mt-1 text-sm text-gray-600">{p.broker.fullName}</p>
                  {p.broker.phone && <p className="text-sm text-blue-600">{p.broker.phone}</p>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 rounded-2xl border border-gray-200 bg-white p-6">
              ...
              <p className="mt-1 text-sm text-gray-500">Quan tâm đến bất động sản này?</p>

              {leadSent ? (
                <div className="mt-4 rounded-xl bg-green-50 p-4 text-center">
                  <p className="text-sm font-medium text-green-700">Gửi liên hệ thành công!</p>
                  <p className="mt-1 text-xs text-green-600">Môi giới sẽ liên hệ với bạn sớm nhất.</p>
                </div>
              ) : (
                  ...
                  <div className="relative h-40 bg-gradient-to-br from-gray-400 to-gray-600">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-4xl opacity-30">
                        {s.type === 'apartment' ? '🏢' : s.type === 'house' ? '🏠' : s.type === 'villa' ? '🏡' : '🗺️'}
                      </span>
                    </div>
                  </div>
                  <div className="p-4">
                    <h3 className="font-semibold text-gray-900 group-hover:text-blue-600 line-clamp-1">{s.title}</h3>
