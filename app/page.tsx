    ...
    async function fetchData() {
      try {
        const [hotData, propsData] = await Promise.all([
          api.getHotProperties(4),
          api.getProperties({ limit: 8 }),
        ]);
        // Backend returns array directly, or { data: [...], metadata: {...} }
        setHotProperties(Array.isArray(hotData) ? hotData : []);
    ...
    <div>
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-4 text-lg text-blue-100">
              Kết nối người mua, người bán và môi giới trên nền tảng bất động sản hàng đầu Việt Nam.
              Hỗ trợ bởi AI Recommendation.
            </p>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">Được nhiều người quan tâm nhất</p>
              </div>
            ...
            <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {hotProperties.map((property: any) => (
      ...
      <section className="bg-gray-50 py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Danh sách bất động sản</h2>
      ...
      <section className="py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-center text-2xl font-bold text-gray-900">Tại sao chọn Nexus Estate?</h2>
          <div className="mt-10 grid gap-8 sm:grid-cols-3">
            {[
              { icon: '🤖', title: 'AI Recommendation', desc: 'Gợi ý bất động sản phù hợp nhất dựa trên hành vi và sở thích của bạn.' },
              { icon: '🔍', title: 'Tìm kiếm thông minh', desc: 'Tìm kiếm nhanh chóng với nhiều bộ lọc và hỗ trợ ngôn ngữ tự nhiên.' },
              { icon: '🛡️', title: 'An toàn & Tin cậy', desc: 'Xác thực thông tin người dùng, bảo vệ giao dịch của bạn.' },
            ].map((feature) => (
              <div key={feature.title} className="rounded-xl border border-gray-200 p-6 text-center">
      ...
      <section className="bg-blue-600 py-16">
        <div className="mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-white">Bạn là môi giới bất động sản?</h2>
          <p className="mt-2 text-blue-100">Đăng tin và tiếp cận hàng ngàn khách hàng tiềm năng.</p>
          <Link
            href="/signup"
...
function PropertyCard({ property, formatPrice }: { property: any; formatPrice: (p: number) => string }) {
  const thumbnailColors = ['from-blue-400 to-blue-600', 'from-emerald-400 to-emerald-600', 'from-purple-400 to-purple-600', 'from-amber-400 to-amber-600'];
  const colorClass = thumbnailColors[Math.floor(Math.random() * thumbnailColors.length)];
