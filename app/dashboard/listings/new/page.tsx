...
export default function NewListingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const propertyResponse = await api.createListing({
        property: {
          title: form.title,
      ...
      <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
        <div className="w-full max-w-md text-center">
          <div className="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
            ...
            <h2 className="mt-4 text-xl font-bold text-gray-900">Đăng tin thành công!</h2>
            <p className="mt-2 text-sm text-gray-500">Đang chuyển về Dashboard...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6">
          <Link href="/dashboard" className="text-sm text-blue-600 hover:underline">&larr; Quay lại Dashboard</Link>
          <h1 className="mt-2 text-2xl font-bold text-gray-900">Đăng tin mới</h1>
          <p className="text-sm text-gray-500">Nhập thông tin bất động sản cần đăng</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 space-y-5">
            ...
            <div>
              ...
              <input type="number" required value={form.price} onChange={handleChange('price')}
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input type="number" value={form.area} onChange={handleChange('area')}
                  className="mt-1 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm outline-none focus:border-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Phòng ngủ</label>
                <input type="number" value={form.bedrooms} onChange={handleChange('bedrooms')}
                  className="mt-1 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm outline-none focus:border-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Phòng tắm</label>
                <input type="number" value={form.bathrooms} onChange={handleChange('bathrooms')}
                  className="mt-1 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm outline-none focus:border-blue-500"
                />
              </div>
            </div>
            ...
            <h2 className="text-lg font-semibold text-gray-900 pt-2">Địa chỉ</h2>
