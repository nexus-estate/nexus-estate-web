...
API/PostgreSQL lifecycle. It requires Docker and a sibling API checkout whose
repository, branch, and SHA match the expected CI ref (default:
`origin/develop`). Use `NEXUS_API_ROOT` for another checkout path or
`NEXUS_API_EXPECTED_REF` for an explicitly selected API commit; the gate
reports and validates both revisions without changing the API repository.
